...
struct TreeNode *recurBuildTree(int* preorder, int* inorder, int left, int right, int* idx, struct TreeNode* pool, int* poolIdx)
{
    if (left >= right)
        return NULL;
    ...
    root->left = recurBuildTree(preorder, inorder, left, div, idx, pool, poolIdx);

    // right subtree is from index that we found to right
    root->right = recurBuildTree(preorder, inorder, div+1, right, idx, pool, poolIdx);
...
struct TreeNode* buildTree(int* preorder, int preorderSize, int* inorder, int inorderSize){
    ...
    return recurBuildTree(preorder, inorder, 0, preorderSize, &idx, pool, &poolIdx);
