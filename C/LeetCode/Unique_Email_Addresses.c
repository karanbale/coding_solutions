...
For example, "alice.z@leetcode.com" and "alicez@leetcode.com" forward to the same email address. (Note that 
...
void numUniqueEmails_Method2(char ** emails, int emailsSize){

    int uniqueEmails = emailsSize;
    for(int currEmail=0; currEmail<emailsSize; currEmail++){
        for(int nextEmail=1; nextEmail<emailsSize; nextEmail++){
            int char1 =0, char2 = 0;
            while((*(*(emails+currEmail)+char1) != '\0') ||
                  (*(*(emails+nextEmail)+char2) != '\0')){
                      // check if its '+' char, skip over and go to next char
                      // check if its 
                  }
        }
    }
...
int numUniqueEmails_Method1(char ** emails, int emailsSize){
...
int main() 
{
    ...
    // printf("output: %d\n", numUniqueEmails_Method2(email, strlen(*email)));
    numUniqueEmails_Method2(email, strlen(*email));
}